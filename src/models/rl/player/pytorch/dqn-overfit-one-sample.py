...
def select_action(case, validation=False, random_=False):
    ...
    steps_done += 1
    if (not random_) and ((validation) or (sample > eps_threshold)):
        with torch.no_grad():
            q_action = policy_net(state)
...
def plot_durations():
    ...
    ax.axhline(y=train_reward_target, color='r', linestyle='-', label='Max achievable reward')
    ...
    # ax.set_title('Validation Rewards every {} episodes'.format(TARGET_UPDATE))
    # ax.set_xlabel('Episode')
    # ax.set_ylabel('Reward')
    ...
    #     ax.set_xticks(x)
    #     xlabels = [str(i) for i in x*TARGET_UPDATE]
...
for i in range(FILL_MEM):
    ...
    for t in count():
        ...
        actions.append(actions_dict[action.data.tolist()[0][0]])
        next_case, reward, done = env.step(action.item())
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        next_case, reward, done = env.step(action.item())
        print(case.settings, next_case.settings, reward)
        reward = torch.tensor(np.array([reward]).astype(np.float32), device=device)
        ...
        if done:
            print('----------')
            print('Target reward: ', train_reward_target_0)
            print(' ')
            print('Case: ', case.reward, case.settings, actions)
            print('----------')
