...
baseline_val_path = './baseline_val.pk'
...
features = features.set_index('id_gaia')

train_ids = pd.read_csv(train_path, header=None)
val_ids = pd.read_csv(val_path, header=None)

train_features = features.loc[train_ids.values.reshape(-1)]
val_features = features.loc[val_ids.values.reshape(-1)]

train_features = train_features.reset_index()
val_features = val_features.reset_index()

target_reward = pd.read_csv(reward_path)

with open(baseline_val_path, 'rb') as file:
    cases_val = pickle.load(file)
baseline_val = np.mean([c.reward for c in cases_val])
...
for i_episode in range(num_episodes):
    ...
    if i_episode % VAL_METRICS == 0:
        ...
        _, val_rewards_all = get_reward(val_cases)
        val_rewards.append(np.mean(val_rewards_all))

        '''Random'''
        _, val_rewards_rand_all = get_reward_rand(val_cases)
