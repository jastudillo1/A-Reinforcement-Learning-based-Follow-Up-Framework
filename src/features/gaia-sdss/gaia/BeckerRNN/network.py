...
class Network():
    ...
    def add_model(self):
        ...
        with tf.compat.v1.variable_scope('RNN_'):
            ...
            self.all_h, state = tf.nn.dynamic_rnn(stacked_gru, self.data_pl, dtype=tf.float32, sequence_length=self.length_pl, swap_memory=True)

        with tf.compat.v1.variable_scope('Cell_Values'):
            self.last_h = self.last_relevant(self.all_h, self.length_pl)
    ...
    def predict(self, tfrecords, model_name, metadata_train_path, return_h=False, return_fc=False, return_p=False):
        ...
        with tf.compat.v1.Session() as sess:
            ...
            tensors = [self.all_h, self.last_h, self.fc, self.prediction]
            ...
            while True:
                try:
                    ...
                    all_h, last_h, fc_output, pred_probs = sess.run(tensors, feed_dict)
                    all_h = [h[:l] for h,l in zip(all_h, lengths)]

                    ### Acumulate results
                    _preds = np.append(_preds, pred_probs.argmax(1))
                    _labels = np.append(_labels, labels.argmax(1))
                    _lengths = np.append(_lengths, lengths)
                    _ids = np.append(_ids, ids)
                    _all_h = _all_h + all_h
            ...
            self.predictions = {'ids': _ids, 'labels': labels, 'pred_label': pred_label, 'pred_probs': pred_probs
                            , 'last_h': _last_h, 'fc_output': _fc_out, 'all_h': _all_h, 'lengths': _lengths}
